...
public class ProductDao {
	...
	public static <T> PreparedStatement getPreparedInsert(Connection conn, String tablename, T currentEntity)
			throws SQLException
	{
		Product product = (Product) currentEntity;
		PreparedStatement ps = conn.prepareStatement("INSERT INTO " + tablename
				+ " (idproducts, name, description, price, `show`, stores_idstores) VALUES (?, ?, ?, ?, ?, ?)");
	...
	public static <T> PreparedStatement getPreparedUpdate(Connection conn, String tablename, T currentEntity)
			throws SQLException
	{
		Product product = (Product) currentEntity;
		PreparedStatement ps = conn.prepareStatement("UPDATE " + tablename
				+ " SET name = ?, description = ?, price = ?, `show` = ?, stores_idstores = ? WHERE idproducts = ?");
